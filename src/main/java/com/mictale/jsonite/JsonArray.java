 ...
 * iterate over the array and {@link #rewind()} to reset the cursor.
 * 
...
public final class JsonArray extends JsonValue implements List<JsonValue> {
	...
	protected final List<JsonValue> elements;

	private int position;
	
	protected JsonArray(List<JsonValue> elements) {
	...
	private JsonArray(JsonValue[] a) {
		this(new ArrayList<>(Arrays.asList(a)));
	}

	/**
	 * Creates a new array initialized with the specified elements.
	 * 
	 * @param value contains the initial elements.
	 * @return the array.
	 */
	public static JsonArray of(JsonValue value) {
    ...
    /**
     * Retrieves the current position in this array.
     *
     * Call {@link #hasNext()} to iterate, {@link #next()} to retrieve the element at this position
     * and {@link #rewind()} to reset the position.
     *
	...
	public boolean equals(Object obj) {
		if (obj instanceof JsonArray) {
			...
			return other.size() == size() && elements.containsAll(other.elements);
		}
		else {
			return false;
		}
	...
	private final class Iter implements Iterator<JsonValue> {
